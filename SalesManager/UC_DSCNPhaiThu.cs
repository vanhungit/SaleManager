...
{
    public partial class UC_DSCNPhaiThu : UserControl
    {
        public UC_DSCNPhaiThu()
        {
            InitializeComponent();
            ...
            SqlDataAdapter AdapterStockInward = new SqlDataAdapter("SELECT	d.RefID AS ChungTu,p.Customer_ID, p.CustomerName, d.RefDate AS Ngay,	dbo.CallExpire(d.RefDate,GETDATE()) AS TuoiNo,	d.Amount AS SoTien, d.Payment AS DaTra,	d.Balance ConLai, d.[Description] AS DienDai FROM DEBT d INNER JOIN CUSTOMER p ON d.CustomerID = p.Customer_ID	WHERE d.CustomerID IN (SELECT p.Customer_ID FROM CUSTOMER p) AND d.Balance<>0 and DATEDIFF(day,'" + dateTu.DateTime + "' ,d.RefDate)>=0 and DATEDIFF(day,'" + dateDen.DateTime + "',d.RefDate)<=0 ORDER BY p.CustomerName", DataProvider.ConnectionString);
            SqlDataAdapter AdapterStockInwardDetail = new SqlDataAdapter("select e.ChungTu ,S.Product_ID,S.ProductName,S.Unit,S.Quantity,S.UnitPrice,S.Amount from (SELECT d.RefID AS ChungTu, p.CustomerName, d.RefDate AS Ngay FROM DEBT d INNER JOIN CUSTOMER p ON d.CustomerID = p.Customer_ID	WHERE d.CustomerID IN (SELECT p.Customer_ID FROM CUSTOMER p) and DATEDIFF(day,'" + dateTu.DateTime + "' ,d.RefDate)>=0 and DATEDIFF(day,'" + dateDen.DateTime + "',d.RefDate)<=0 AND d.Balance<>0 ) e, STOCK_OUTWARD_DETAIL S	where e.ChungTu = S.Outward_ID", DataProvider.ConnectionString);
