...
{
    public partial class UC_CongNoDauKyNPP : UserControl
    {
        DataTable dtable = new DataTable();
        SYS_USER objuser = new SYS_USER();
        frmSoDuDauKy frmsodudauky;
        public UC_CongNoDauKyNPP(frmSoDuDauKy _frm)
        {
            InitializeComponent();
            ...
            gridControl1.DataSource = PROVIDER_DEBT_GetListInit(gridLookUpEdit1View.GetFocusedRowCellDisplayText("Customer_Group_ID"));
            frmsodudauky = _frm;
        }
        ...
        public string ThemCongNoNPP(string SOID, string ID_Provider, double Amount, string User)
        {
            string Trave = "0";
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(consString))
                {
                    using (SqlCommand cmd = new SqlCommand("CUSTOMER_DEBT_Update"))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@RefType", 32);
        ...
        public DataTable PROVIDER_DEBT_GetListInit(string Khuvuc)
        {
            SqlConnection con = new SqlConnection(DataProvider.ConnectionString);
            SqlCommand sqlcmd = con.CreateCommand();
            sqlcmd.CommandText = "exec PROVIDER_DEBT_GetListInit @Customer_Group_ID=N'" + Khuvuc + "'";
            SqlDataAdapter da = new SqlDataAdapter();
            da.SelectCommand = sqlcmd;
            DataSet ds = new DataSet();
            //con.Open();
        ...
        {
            //MessageBox.Show(gridView1.GetRowCellValue(gridView1.FocusedRowHandle, gridView1.Columns["ID"]).ToString());
        }
                    ...
                    try
                    {
                        for (int i = 0; i < gridView1.RowCount; i++)
                        {
                            if ((gridView1.GetRowCellValue(i, gridView1.Columns["ID"]).ToString() == "11111111-1111-1111-1111-111111111111") && (gridView1.GetRowCellValue(i, gridView1.Columns["Amount"]).ToString() !=""))
                            {
                                ...
                                ThemCongNoNPP("NDK-" + gridView1.GetRowCellValue(i, gridView1.Columns["Customer_ID"]).ToString(), gridView1.GetRowCellValue(i, gridView1.Columns["Customer_ID"]).ToString(), double.Parse(gridView1.GetRowCellValue(i, gridView1.Columns["Amount"]).ToString()), objuser.UserID);
                            }
                        }
