...
{
    public partial class UC_BaoCaoBanHangTheoKH : UserControl
    {
        public UC_BaoCaoBanHangTheoKH()
        {
            InitializeComponent();
            ...
            SqlDataAdapter AdapterStockInward = new SqlDataAdapter("select Customer_ID,CustomerName,sum(d.DetailDT) as DoanhThu from (select Outward_ID,	Sum(Amount) as DetailDT from STOCK_OUTWARD_DETAIL group by Outward_ID) d , STOCK_OUTWARD s where d.Outward_ID = s.ID and DATEDIFF(day,'" + dateTu.DateTime + "' ,RefDate)>=0 and DATEDIFF(day,'" + dateDen.DateTime + "',RefDate)<=0 group by Customer_ID,CustomerName", DataProvider.ConnectionString);
            SqlDataAdapter AdapterStockInwardDetail = new SqlDataAdapter("select Customer_ID,RefDate,ID,DiscountRate,Discount,Vat,VatAmount,d.ThanhTien from (select Outward_ID,Sum(Amount) as ThanhTien from STOCK_OUTWARD_DETAIL group by Outward_ID) d, STOCK_OUTWARD s where d.Outward_ID = s.ID and DATEDIFF(day,'" + dateTu.DateTime + "' ,RefDate)>=0 and DATEDIFF(day,'" + dateDen.DateTime + "' ,RefDate)<=0 ", DataProvider.ConnectionString);
            ...
            gridView1.Columns["DoanhThu"].DisplayFormat.FormatType = DevExpress.Utils.FormatType.Numeric;
            gridView1.Columns["DoanhThu"].DisplayFormat.FormatString = "n0";
            ...
            gridView2.Columns["DiscountRate"].Caption = "Chiết Khấu (%)";
            gridView2.Columns["DiscountRate"].DisplayFormat.FormatType = DevExpress.Utils.FormatType.Numeric;
            gridView2.Columns["DiscountRate"].DisplayFormat.FormatString = "p";
            gridView2.Columns["Discount"].Caption = "Chiết Khấu";
            gridView2.Columns["Vat"].Caption = "VAT (%)";
            gridView2.Columns["Vat"].DisplayFormat.FormatType = DevExpress.Utils.FormatType.Numeric;
            gridView2.Columns["Vat"].DisplayFormat.FormatString = "p";
            gridView2.Columns["VatAmount"].Caption = "VAT";
